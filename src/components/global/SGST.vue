<template>
    <div class="mealtype pa-6">
         <v-data-table
            ...
            <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>SGST</v-toolbar-title>
          <v-spacer></v-spacer>
     <v-text-field
        v-model="search"
      ...
      <v-dialog
          ...
          <v-dialog v-model="dialogDelete" max-width="350px">
            <v-card>
              <v-card-title class="subtitle-2">Are you sure you want to delete this SGST?</v-card-title>
              <v-card-actions>
                <v-spacer></v-spacer>
...
export default {
    ...
    computed:{
        formTitle () {
        return this.editedIndex === -1 ? 'New SGST' : 'Edit SGST'
      },
        getheaders(){
            //this.role = '';
            let headers= [
    ...
    created(){
        this.role ='admin'; 
        this.items = this.getSGST();
    },
    methods:{
      ...mapGetters(['getSGST']),
      ...mapActions(['addNewSGST']),
        editItem(val){
