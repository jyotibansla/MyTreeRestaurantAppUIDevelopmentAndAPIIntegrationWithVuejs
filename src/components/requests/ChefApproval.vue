<template>
  <div class="chefsRequest pa-4">
    <div class="text-h6">Chef Approval</div>

    <v-data-table
      :headers="headers"
      :items="requestedchefs"
      sort-by="ProductName"
    ...
    >
      <template v-slot:[`item.actions`]="{ item }">
        <div class="d-flex justify-space-between" no-gutters>
          <v-btn color="success" small dark
            class="ma-1 btn" @click="approve(item)">
            Approve
          </v-btn>
          <v-btn color="red" small dark
            class="ma-1 btn" @click="reject(item)">
            Reject
          </v-btn>
        </div>
      </template>
      ...
      <template v-slot:[`item.cancelledCheque`]="{ item }">
          ...
          <v-img
            ...
            width="30px"
          ></v-img>
         <a  @click="download(item.addressProof)">{{ item.cancelledCheque }}</a>
        </span>
      </template>
...
export default {
  ...
  computed: {
    headers() {
      let headers = [
        { text: "Chef Id", sortable: false, value: "chefId", width:'80px' },
        { text: "Name", value: "name", width:'180px' },
        { text: "EmailAddress", value: "email", width:'200px' },
        { text: "Address", value: "address", width:'250px' },
        { text: "City", value: "city" , width:'120px'},
        { text: "Pincode", value: "zipcode", width:'120px' },
        { text: "AddressProof", value: "addressProof", width:'200px' },
  ...
  created() {
    this.unsubscribe = this.$store.subscribe((mutation) => {
      if (mutation.type === "setAllRequestedChefs" && mutation.payload) {
        this.requestedchefs = JSON.parse(mutation.payload);
      }
    });
    this.getRequestedChefs();
  },
  methods: {
    ...mapActions(["getRequestedChefs","approveRequestedChef","rejectRequestedChef"]),
    approve(item) {
      let param = {
        chefId : item.chefId
      };
      this.approveRequestedChef(param);
    },
    reject(item) {
       let param = {
        chefId : item.chefId
      };
      this.rejectRequestedChef(param);
    },
